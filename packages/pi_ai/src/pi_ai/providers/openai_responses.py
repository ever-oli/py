...
Python port of TypeScript providers/openai-responses.ts
"""
...
class OpenAIResponsesOptions:
    """Options for OpenAI Responses API."""
...
def stream_openai_responses(
    ...
    """Stream completions from OpenAI Responses API."""
        ...
        try:
                ...
                try:
                    async with httpx.AsyncClient() as client:
                        async with client.stream(
                            ...
                            raise Exception(
                                f"OpenAI Responses API error ({resp.status_code}): {error_str}"
                            )
            ...
            async for line in response.aiter_lines():
                ...
                if event_type == "response.created":
                    ...
                    raise Exception(
                        f"OpenAI error: {error.get('code')}: {error.get('message', 'Unknown error')}"
                    )
                    ...
                    if error:
                        raise Exception(
                            f"OpenAI response failed: {error.get('code')}: {error.get('message', 'Unknown error')}"
                        )
                    details = event.get("response", {}).get("incomplete_details", {})
                    if details:
                        raise Exception(
                            f"OpenAI response incomplete: {details.get('reason', 'Unknown reason')}"
                        )
                    raise Exception("OpenAI response failed: Unknown error")
...
def stream_simple_openai_responses(
    ...
    """Simple streaming interface for OpenAI Responses API."""
